...
>   - **카카오 선물하기** — ✅ 1차 출시(5/26) · ✅ **2차 출시 TODO-027 완료(6/12, 20종 조기 출시)**. 잔여: TODO-029 운영가이드(초안 6/15) · TODO-041 어드민 쿠폰내역 노출 · TODO-013·026 핫픽스 잔여 · TODO-043 굿즈 문의
...
> 6/12 금 — **이번 주 = SDK 이관 + 카카오 2차 동시 파도**. ✅ TODO-027 2차 출시 완료(6/12, 조기) · 🔥 **6/12(오늘) TODO-035 백필·캠페인 + TODO-012 개발완료 + TODO-040 스레드** · 6/15 TODO-035 서버·앱 배포 + TODO-005 Azure 계약 PDF 전달 + TODO-029 가이드 초안.
...
| 2 | TODO-012 | ⭐⭐ | 홈탭 Phase #2 — 랭킹 자동화 | 📅 **6/12 (금) 개발 완료** (6/5 기획·설계 → 6/12 개발로 연장). S5(/architect) → /dev-* |

### 2순위 — 이번 주~다음 주 마감 (6/12~6/15)

| # | ID | 중요 | 제목 | 액션 |
|---|----|------|------|------|
| 3 | TODO-029 | ⭐⭐⭐ | 카카오 선물하기 운영 가이드 문서 | 🟢 **진행** — 문서 0·1·2·6 초안 완료. 다음: 3·4·5 + doc4/FAQ 중복정리 + 식별자 확인. **초안 6/15** |
| 4 | TODO-005 (Azure) | ⭐⭐⭐ | Azure 계약 이전 | 🔥 **6/15 (월)** — MS 승인 → 계약 정보 수기 작성 PDF 윤홍욱 매니저 전달 |
| 5 | TODO-040 | ⭐⭐ | 2026 3Q/4Q Bottom-Up 이니셔티브 제안 스레드 | 📅 **6/12 (금)** — 스레드 생성 + 링크 기입 → 제안 작성. ⚠ 링크 미기입 |
| 6 | TODO-042 | ⭐⭐ | MWAA 현황 파악 (인기차트 배치 위치 결정) | TODO-012 배치 위치(D-7) 선결 — hellobot-mwaa DAG 인벤토리 + 역할 분담. /dev-infra·/dev-data 위임 후보 |
| 7 | TODO-041 | ⭐⭐ | 카카오 어드민 — 쿠폰 사용 내역 식별 정보 노출 | /dev-server — 스킬명/ProductCode/사용자명 컬럼 추가. TODO-029 연계 |

### 3순위 — 인프라 잔여 / 6월 내 결판 / 외부 추적

| # | ID | 중요 | 제목 | 액션 |
|---|----|------|------|------|
| 8 | [gcp-gemini-cost-mitigation](projects/20260519-gcp-gemini-cost-mitigation/) | ⭐⭐⭐ | 3-트랙 (A IP제한 / B 메가존 / 3 보안점검) | ⚠ **5/31 결판 지남** — A·B 외부 회신 상태 확인 + 무응답 시 재독촉 |
| 9 | TODO-033 | ⭐⭐⭐ | 비트윈 인프라 의존성 인벤토리·정리 | 📅 **6월 중순** — Phase A 콘솔 인벤토리 + Phase B 딜라이트룸 1차 메일 |
| 10 | TODO-007 | ⭐⭐⭐ | 비트윈 인프라 분리 (DLT) — umbrella | sub 019 ✅(6/5) — **033만 남음 → 클로즈 임박** |

### 4순위 — 개인화 기획·핫픽스 잔여

| # | ID | 중요 | 제목 | 액션 |
|---|----|------|------|------|
| 11 | TODO-044 | ⭐⭐ | 개인화 추천 액션 아이템 기획 및 리뷰 (cross-sell 후속) | 🟢 트랙 B 여정 프레임(3그룹 상세, journey-action-map + 덱 14장) → 트랙 A 미팅 일정 |
| 12 | TODO-013 | ⭐⭐⭐ | 카카오 핫픽스 잔여 (umbrella) | ⚠ 5/22 결정 마감 지남 — 1차 출시(5/26) 완료 → 잔여 정합화 일정 결정 |
| 13 | TODO-026 | ⭐⭐⭐ | 스킬 교환권 상품↔쿠폰 정합성 (ISS-071 P0) | ⚠ 5/22 결정 마감 지남 — 결함 9건 처리 방향 결정 |
| 14 | TODO-022 | ⭐⭐ | 운영 어드민 사용취소 (ISS-056 P0) | ✅ 5/21 배포 — 정합화 일정 TODO-013 내 |

### 5순위 — 분석 · 장기 누적

| # | ID | 중요 | 제목 | 액션 |
|---|----|------|------|------|
| 15 | TODO-021 | ⭐⭐ | 구매자수·매출 YoY 하락 원인 분석 | Phase 6(A) Activation Funnel (다음 /dev-data) |
| 16 | TODO-016 | ⭐⭐ | 카카오 선물하기 슬랙 알림 | ⚠ 5/20 초과 — workspace:8 cmux 세션 상태 확인 |
| 17 | TODO-032 | ⭐⭐ | accadmin 2중인증 채널·번호 변경 | 사용자 확인 5건 → 트랙1(채널)·트랙2(번호) |
| 18 | TODO-038 | ⭐⭐ | Google AI API 키 보안 점검 | T-1 레퍼러 제한 / T-2 llm-prod / T-3 나머지 |
| 19 | TODO-025 | ⭐ | 데이터 백로그 (umbrella) | 현재 3건: 리뷰 수집·분석 / 스킬별 특성 구분 / 첫 구매 스킬별 리텐션·LTV |
| 20 | TODO-045 | ⭐⭐ | [PMI] Slack 알림 기능 이전 (A→B 워크스페이스) | 🆕 조사 완료 — 선행: B에 레거시 웹훅(채널 override) 생성 가능 여부 확인 → 채널 ~10개·시크릿 2곳·subteam 코드 수정 |
...
| 24 | TODO-046 | ⭐ | 하트 잔액 이상치 정리 (heart-balance 후속) | ⏸ 백로그 — 재개 시 A·B 26명 user_test_group 반영부터 |
