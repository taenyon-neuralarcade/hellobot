...
프로젝트 단위가 아닌 일상 업무는 TODO 시스템으로 추적합니다 (운영 규칙: [CLAUDE.md §할 일 관리](../CLAUDE.md#할-일-관리-todomd--todos)):

| 층 | 위치 | 역할 |
|----|------|------|
| **인덱스** | [`TODO.md`](../TODO.md) | 표 형식 한 줄 인덱스 + 최상단 🎯 우선순위 보드 (매일 갱신) |
| **상세** | [`todos/TODO-NNN-{slug}.md`](../todos/readme.md) | TODO별 컨텍스트·진행 로그·현재 상태·다음 단계 — 세션을 넘나드는 재개용 |

작업 재개 시 상세 파일의 `현재 상태` → `다음 단계`만 읽고 이어갑니다. TODO가 커지면 프로젝트로 승격(`프로젝트로 추적` 섹션)합니다.
...
각 워크트리는 독립적이므로 코드 영역이 다르면 서로 영향 없이 작업할 수 있습니다. 다만 공용 SSOT 문서(데이터 카탈로그, 워크스페이스 문서, 번호 카운터)는 동시 변경 시 머지 충돌이 자주 발생하므로, 직렬화에 가까운 운영 패턴이 필요합니다. 상세: [CLAUDE.md §동시 작업과 충돌 회피](../CLAUDE.md#동시-작업과-충돌-회피).
