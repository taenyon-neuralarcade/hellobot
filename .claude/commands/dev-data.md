...
### 데이터 카탈로그 위치
`common-data-airflow/docs/hellobot-data/catalog/` (리포 내부, 단일 진실 원천)
...
## 카탈로그 갱신 프로토콜 (작업 중 새 사실·룰 발견 또는 기존 표현 오류 발견 시) ★★

> **1순위 루틴**: `/dev-data` 의 본 프로토콜은 다른 어떤 산출물보다 우선합니다. 작업·탐색·검증 과정에서 카탈로그(SSOT)에 명시되지 않은 사실을 **새로 인지** 했거나, 기존 표현이 **사실과 다름을 발견** 했다면 — 그 두 케이스는 동일 우선순위로 다룹니다 — 본 절차로 즉시 진입합니다. 발견을 기억에만 두고 다음 작업으로 넘어가는 것은 SSOT 가 어긋나는 가장 흔한 원인입니다 (이번 세션 ISS-011·012·014·015·016·017 이 모두 그 결과).

### 핵심 원칙

- **신규 등록 = 기존 오류 정정** — 가중치 동일. 정정도 발견 즉시 처리.
- **자체 발견도 트리거** — 사용자 지적이 없어도, 본인이 BQ/코드/문서 비교 중 인지한 사실도 동일하게 진입.
- **세션 종료 후 일괄 처리 금지** — 발견 즉시 한 호흡 안에서 갱신 제안·승인·반영까지 마칠 것. 미루는 순간 휘발됨.
- **출처 명시 필수** — 모든 갱신은 출처(쿼리·실행일·스캔 바이트 / 코드 라인 / 사용자 발화 / 외부 문서) 가 따라가야 검증 가능.

### 트리거 (인지 즉시 본 흐름 진입)

새 사실 발견:
- 사용자가 정책·운영 룰을 알려줌 (예: "env 필터는 prod 만", "ID 보내면 이름도 함께")
- BQ 직접 조회로 카탈로그에 없던 스키마·파티션·실 분포 확인
- 외부 문서 (Notion 설계 DB, 슬랙, 다른 리포 PR) 에서 새 컨벤션 발견
- 다른 에이전트(`/architect`, `/qa`, `/review`) 산출물에서 새 합의 발견

기존 표현 오류 발견 (정정 트리거 — 이전 세션이 잘못 적은 사실 포함):
- BQ 실측 결과와 카탈로그 표현이 다름 (테이블 위치·컬럼·타입·파티션·row 수 등)
- 코드 분석 결과와 카탈로그 표현이 다름 (DAG 동작·소스 시스템·적재 경로 등)
- 카탈로그·코드 간 사실 불일치 (ISS-NNN 후보)
- 카탈로그 내부 문서 간 표현 불일치 (한쪽이 stale)
...
> 이번 작업·탐색 도중 (1) 새 사실을 인지했거나 (2) 카탈로그의 기존 표현이 사실과 다름을 발견했나? → 인지 즉시 본 §카탈로그 갱신 프로토콜 진입. 두 경우 모두 동일 우선순위로 처리.

또한 다음 시점에는 항상 점검:
- BQ `bq show` / 쿼리 결과 확인 직후
- 코드(DAG · scripts) 분석 직후
- 산출물 작성 직전 (보고 응답을 마무리하기 전 마지막 점검)

### 금지

- ❌ 룰·사실을 인지하고 사용자에게 보고만 한 채 카탈로그 미갱신 — 다음 세션에서 다시 어긋남
- ❌ **기존 표현이 틀렸음을 발견하고도 정정 제안을 빠뜨림** — 새 사실 등록만 처리하고 stale 표현은 그대로 두는 패턴 (ISS-017 같은 누적 부채의 원인)
...
> ⚠️ `analytics_164027297.server_events` 의 파티션 컬럼은 **`event_timestamp` (TIMESTAMP, DAY)** — `event_date` 컬럼은 존재하지 않음. 단순 timestamp 범위(`event_timestamp >= ... AND <`)도 동작하나 위 패턴이 가장 효율적 (검증: 0.9MB vs 20MB, 2026-04-27 dry-run). 상세: catalog [ISS-012](../../common-data-airflow/docs/hellobot-data/catalog/issues.md).
>
...
- ❌ deprecated 된 `docs/hellobot-data/tables/` 의 컬럼 가정으로 쿼리 작성 (실제 스키마 `bq show` 또는 SQL 파일로 먼저 확인)
...
```
필수 읽기 (모든 과업):
  1. common-data-airflow/docs/hellobot-data/catalog/infra-map.md
...
금지:
  - **기존 common-data-airflow/docs/hellobot-data/tables/ 참조 금지** (deprecated, ISS-001)
