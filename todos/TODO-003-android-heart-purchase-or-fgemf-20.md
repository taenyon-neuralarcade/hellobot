...
**유형**: 액션 (D&B 회신 확인 후 모니터링으로 전환 예정)
**상태**: 진행 중 (NICE D&B에 DUNS 주소지 업데이트 긴급 요청 완료 — 5/14 오전 회신 대기)
**등록**: 2026-05-13
**시작**: 2026-05-13
**완료**: -
**마감**: - (5/11~ 매출 손실 지속, D&B→Google 동기화 최대 5영업일 추가 소요)
**담당**: 사용자 직접 (D&B 채널 + Play Console 재인증) + 코디네이터 추적
**관련**: 아직 프로젝트화 안 함 — 처리 지연 시 검토
...
**원인 (정정)**: Google Play Console 결제 프로필의 **조직(회사) 정보 변경(주소 등)이 미반영**되어 본인 인증 상태 불일치 발생 → Google이 결제 자동 거절(OR-FGEMF-20). 초기 추정인 "KYB 검증 미완료"는 정확하지 않음 — KYB 자체는 완료된 상태이고, **변경된 사업자 정보가 D&B → Google 경로로 동기화되지 못한 것**이 정확한 원인.

**왜 Play Console 펜(연필) 아이콘으로 직접 수정이 안 되는가**:
- 조직(회사) 계좌 + 본인 인증 완료 상태에서는 결제 프로필에서 조직명/주소를 직접 변경 불가
- 조직 계정의 조직명/주소 SSOT는 Google이 아니라 **Dun & Bradstreet(D&B)**. Google Play 개발자 계정은 D&B 프로필을 진실의 원천으로 참조함
- 한국 사업자는 보통 **NICE D&B(나이스디앤비)** 를 통해 DUNS 정보를 관리

**진행 중인 조치 (사용자 직접 수행 중)**:
- NICE D&B에 **DUNS 주소지 정보 업데이트 긴급 요청 완료**
- D&B로부터 **2026-05-14 오전까지 업데이트 회신 약속** 받음

**현 단계 블로커**: D&B 업데이트 완료 회신(5/14 오전) + 이후 D&B → Google 자동 동기화 (Google Support 가이드 기준 D&B 처리 완료 시점부터 **최대 5영업일**).

영향:
- Android 인앱구매 매출 손실 지속 (5/11~)
- 일부 결제는 통과 중(0건 아님) → 모든 결제가 차단되지는 않은 상태
- 빠르면 5/14 + 5영업일 = ~5/21 까지 손실 가능 (D&B 동기화 속도에 의존)
