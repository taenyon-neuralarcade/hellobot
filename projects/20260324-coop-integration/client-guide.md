...
| 요소 | 데이터 소스 |
|------|-----------|
| 일러스트 | **플랫폼별 기존 하트 충전 애니메이션 자산** (Lottie/GIF 등) 재사용 — design-spec.md §S3 "일러스트 자산 규범" 참조 |
| 캡션 | 이벤트/상품명 (Red400) |
| 제목 | "하트가 {heartQuantity}개 충전되었어요!" |
| 본문 | "하트는 \<프로필\>탭에서 확인 가능해요" |
| 확인 버튼 | 프로필 탭으로 이동 |

> **일러스트 구현 주의 (ISS-035, 2026-04-22)**: 정적 PNG(이전 `img_heart_complete.png`) 사용 금지 — 반드시 **재생되는 애니메이션**. 신규 자산은 발급되지 않으므로 각 플랫폼에서 이미 사용 중인 하트 충전 관련 자산을 재사용합니다. 영역 비율 240×117(≈2.05:1) 유지, 원본 자산 비율과 근사(±2% 이내) 시 `scaleAspectFit`로 맞추고 별도 cropping 금지.
...
> **레이아웃 일관성 (ISS-031, 2026-04-22)**: 스킬 이용권 카드는 **같은 리스트의 일반 쿠폰 카드와 박스 높이·점선 위아래 여백·내부 수직 리듬을 동일**하게 유지합니다. 스킬 이용권에 해당하지 않는 행(서브텍스트 "N원 이상 결제 시", 만료일 등)은 **레이아웃 공간은 그대로 유지(reserve)하고 내용만 미노출**(예: `visibility: invisible` / `alpha 0` / 동일 높이의 빈 placeholder)하는 방식으로 구현하며, 행 자체를 제거(collapse)하지 않습니다. 신규 고정 픽셀 값을 지정하지 말고, 각 플랫폼이 렌더 중인 **일반 쿠폰 카드의 실측 렌더 결과**를 기준으로 동일 높이·여백을 재현합니다. 상세 규범은 design-spec.md §S4 "레이아웃 일관성 규범" 참조.
...
### 중복 탭 방지 (필수) — 등록 버튼 로딩 상태 규범 (ISS-042, 2026-04-22 확정)

- **등록 버튼**: 탭 즉시 **`disabled` + 회색 톤으로 변경**하여 API 응답 대기 중 추가 요청을 차단합니다. 응답(성공/실패) 수신 시점에 활성 상태로 복귀합니다.
  - 회색 톤은 기존 "입력 없음(empty input)" 상태의 비활성 색상 토큰(`#C6C8CC` 배경 + `#EDEDEE` 텍스트)을 **재사용**합니다. 신규 "로딩 전용" 색상 토큰을 만들지 않습니다.
  - **스피너/인디케이터/GIF/풀스크린 오버레이 금지** — 별도의 로딩 애니메이션 요소를 신규 도입하지 않습니다. `disabled` 단독으로 중복 요청 방지 요건을 충족합니다.
