-- C (web-only) master export — REVISION 3 — pay_for_% only, 첫구매 WEB & 앱결제 0 (웹온리)
-- A 분석과 동일 프레임워크. 첫구매일 2024-03-02~2026-03-02 (90일 성숙).
-- ★ REV3 추가: r_* = "첫 다른날(익일+) 90일 내 구매" = 진짜 재방문 SKU
WITH pays AS (
...
agg AS (
  SELECT user_id,
    ...
    COUNT(*) AS total_pays,
    COUNTIF(plat='APP') AS app_rows, COUNTIF(plat='WEB') AS web_rows
...
base AS (
  ...
  WHERE f.f_plat='WEB' AND a.app_rows=0 AND f.f_d BETWEEN DATE '2024-03-02' AND DATE '2026-03-02'
)
