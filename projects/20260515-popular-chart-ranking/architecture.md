...
| `targetSectionTag` | varchar NULL | 칩 복합키 2축 (tagSkills 전용, null=섹션 통째) |
...
| `computedDate` | date | 마트 산출일 (신선도 관측용 — 서빙은 검사 안 함 §1.3) |

- **유니크**: `(rankerId, targetSection, targetSectionTag, rank)` + `(rankerId, targetSection, targetSectionTag, menuSeq)` — tag null 처리(표현식 인덱스 등)는 /dev-server 구현 재량.
...
common-data-airflow 마트 체인 (KST ~11시 산출)
        │  mart_home_skill_ranking (computed_date=오늘)
        ▼
K8s CronJob "load-home-skill-ranking" (12:00 KST = 0 3 * * * UTC, base/hlb/cronjobs/ 선례)
   1. freshness guard: 마트에 computed_date=오늘 행 존재? 없으면 push 생략 + Slack 알람 → 종료(실패)
...
서버 적재 API가 최종 권위로 검증: ① menuSeq를 PG 유효·**노출 가능**(visible) 스킬과 INNER JOIN — 미노출/미존재는 drop + 응답 보고 ② rank 유니크/연속성 검사. BQ 단계에서도 `mart_fixed_menu_server` 기준 사전 제외(이중 방어)하되, **서빙 visibility 필터는 최종 가드로 유지**(일중 비노출 전환 흡수 — over-fetch ×2가 이 간극의 보험, §4).
...
| 시그널 | 가중 | 원천 | 집계 |
|---|---|---|---|
| 구매수 | 0.35 | `mart_use_skill_se` | 7일 합 ÷ **min(7, open_date 경과일, ≥1)** |
...
| 신규부스트 | 0.05 | `mart_skill_open_date_se.event_date` ≤30일 → 1, else 0 (이진, D-4/D-6) | — |

**분모 = 가용일수(CL-08✅ R-3)**: "실데이터 일수"가 아니라 **출시 후 경과일 기준 min(7, 경과일)** — 신규 스킬의 출시 당일 매출 피크가 자연 부스트로 작동(바닥 규칙 없음, 기획 의도 수용). 판매 0인 날도 분모에 포함.
...
| 2 | **랭킹 대상 칩**인가 — 칩 복합키가 랭킹 적재 키에 존재 | AsIs (〃 — 비대상 칩에서 Hackle 노출을 만들지 않아 희석 방지) |
| 3 | FeatureFlag `featured-skills-ranking` (마스터 킬스위치) | off → **전원 C-M** |
...
플래그 2종+단계 구조·네이밍은 키18 선례(`featured-skills-tabs`/`-public-enabled`) 동형 — 운영팀이 이미 아는 롤아웃 절차 재사용. 유저 버킷·일괄(FR-AB3)은 Hackle 유저 단위 분배가 보장.
...
| 상황 | 동작 |
|---|---|
| 랭킹 0건 (한 번도 적재 안 된 키 등) | **AsIs 폴백** + 구조적 로그·메트릭 마킹(variant=C-A인데 AsIs 서빙 — 분석 희석 추적용) + 알람 |
...
| 5 | ②게이팅 = 플래그 2종(`featured-skills-ranking`/`-public-enabled`) + 신규 Hackle 키, 판정 순서 KR→대상 칩→flag→Hackle | 키18 선례 동형·노출 희석 방지 |
