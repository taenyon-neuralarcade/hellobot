...
> 작성: 2026-06-05 (코디네이터, /dev-data 검증 전 1차) · 상태: **v5 (확정값 표기 동기)**
...
> §8 실측 항목은 2026-06-11 /dev-data 실측으로 **일부 완료**(D-5 단위·eval distinct·topic/intents distinct·T1·섹션 값 바인딩) — 잔여·진행은 [tasks.md](tasks.md) 데이터 파트 참조
...
| 7일 윈도우 | 최근 7일 합 ÷ **가용일수** = `GREATEST(1, LEAST(7, DATE_DIFF(END_DATE, open_date, DAY)+1))` | ✅ **CL-08 확정(06-10, ISS-009 정정)** — "실데이터 일수" 아님(판매 0인 날도 분모 포함, 출시 후 경과일 기준·결측 시 7). 콜드스타트 별도 장치 없음 — [architecture §2.2·§2.4](architecture.md) |
...
| 긍정평가비율 | **1 − 💩비율** = `AVG(CASE WHEN eval_emoji='💩' THEN 0 ELSE 1 END)` (별점 부재로 재정의, 0~1) | ⚠️ **CL-06 교정**: 기존 인용 `…weekly.sql` line216은 1~5 평균(다른 식)이라 폐기. ✅ `eval_emoji` distinct **실측 완료(2026-06-11, 5종)** — 식 적용 확정 |
| 신규부스트 | **출시일 `open_date` 30일 이내** 가산(D-4) | 출시일 = `mart_skill_open_date_se.event_date`(D-6). 서버 등록일 기준 폐기(CL-25 반전) |
| 기본 풀 조건(base) | **오리지널 ∧ 유료 ≥750원**(✅ 확정) | 현행 지금인기는 minPrice 60(하트)·별도 임계. ✅ **D-5 단위 바인딩 해소(2026-06-11 실측)**: `menu_price` 단위=하트, **750원=5하트** |
...
popularity_score =
  ...
  + 신규부스트(0|1)           × 0.05
```
- ✅ **`norm()` 확정(CL-04, 2026-06-10)**: **percentile-rank(`PERCENT_RANK()`), 모집단 = 전체 base 풀**(섹션별 아님 — 섹션 간 점수 비교 가능·소풀 왜곡 방지) — [architecture §2.3](architecture.md). norm 방식 교체는 BQ 계산 레이어 한정 비파괴, 데이터 검증·알고리즘 보완은 최종단계 별도 태스크.
- ✅ **긍정평가비율도 측정 5시그널 일괄 percentile 적용**(CL-04 확정과 함께 해소 — norm 제외는 신규부스트(0/1)뿐, architecture §2.3). 위 공식의 일괄 `norm()` 표기 그대로 유효.
...
```sql
-- (개념) 의사 SQL — 초기 검토용 스케치. 실구현 = mart_home_skill_ranking.sql (2026-06-11, PR #188).
-- 윈도우 = 최근 7일(D-6 ~ D-0). 확정값(CL-04✅·CL-08✅): norm=PERCENT_RANK×전체 base 풀, 분모=가용일수(§2 — 아래 COUNT(DISTINCT event_date) 분모는 구식 예시).
...
SELECT menu_seq,
       ...
       +0.15*pos_ratio+0.15*norm(rev7)+0.05*is_new AS popularity_score   -- pos_ratio도 percentile 적용(CL-04✅)
FROM agg /* JOIN ... */;
```
...
| 2026-06-12 | v5 | 코디네이터(/workspace) | **확정값 표기 동기(내용 신설 없음)**: ①CL-04 stale "미확정" 마커 해소 — norm=percentile×전체 base 풀·긍정평가비율 포함 일괄 적용(§5·§9, architecture §2.3 동기) ②CL-08 해소 — 7일 윈도우 분모="실데이터 일수"→**가용일수** `GREATEST(1,LEAST(7,경과일))`(§2, architecture §2.2 동기) ③06-11 실측 완료 반영 — D-5 단위(하트, 750원=5하트)·CL-06 `eval_emoji` distinct 5종(§2) ④헤더 "BQ 인증 만료·미검증" 캐비엇 현행화(실측 일부 완료) |
