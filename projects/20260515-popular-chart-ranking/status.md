...
- 🟠 **A/B 실험 설계 검토 완료(2026-06-10, 사용자 결정 대기)** — PO·데이터분석가 병렬 검토: [planning/ab-test-po-review.md](planning/ab-test-po-review.md)(실험 구조 옵션·의사결정 프레임, **권장=3군 중첩 L/C-M/C-A** — 키18 게이트 흡수+②랭킹 키 중첩) · [planning/ab-test-analysis-design.md](planning/ab-test-analysis-design.md)(지표 체계·검정력 — ✅BQ 실측: 4주 KR 홈 90k·구매자 전환율 p≈0.30·구매자당 ₩19k, **4주 고정 권장**, 2군 MDE 3.2%/3군 4.3%). ⚠️ 사용자 원안 2군(L vs R)은 UI·랭킹 효과 혼재로 **목표2(자동vs수동) 식별 불가**. 결정 대기 = 실험 구조·키18 처리·1차 지표(유저 레벨 net) 승인·dmp 정의 2건 (각 문서 §6/§14).
- ⚪ **S5 설계 위임** — CL-04(norm 방식·모집단)·CL-08(신규 섹션 랭킹). **CL-03 정정(2026-06-08)+정밀화(06-10)**: `featuredSkillsTabs` LIVE 확인 → A 캡처 불요, B=`fetchSkillListBySectionType`(home.ts L540) **후보 fetch 교체**(앱 무변경 — limit+1 선컷으로 재정렬 불충분). 상세 [planning/s5-asis-serving-analysis.md](planning/s5-asis-serving-analysis.md) · [planning/s5-asis-implementation-audit.md](planning/s5-asis-implementation-audit.md).
...
| **현행 컨테이너** (2026-06-08✅·감사 06-10) | `featuredSkillsTabs` 칩 컨테이너 **이미 LIVE**(피쳐 문서↔코드 전수 일치 확인). ①컨테이너 A/B=Hackle키18(legacy recent↔featured) / ②랭킹 A/B=이 프로젝트(미착수). B 주입=`fetchSkillListBySectionType` **후보 fetch 교체**(동일 shape), **앱 계약 무변경** |
