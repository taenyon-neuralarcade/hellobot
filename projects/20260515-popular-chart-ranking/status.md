...
| 서버 | **PR 닫음(06-12) — 재오픈/재생성 대기** | `feat/popular-chart-ranking` (푸시됨·보존) | `worktrees/hellobot-server` (보존) | ✅ 06-11 구현 `7dc5b7bd` + 리뷰 수정 4커밋(`022887d3`~`24bf4350`, tsc 통과). **PR [#2444](https://github.com/thingsflow/hellobot-server/pull/2444) = 06-12 사용자 지시로 close(머지 안 됨)** — 브랜치·커밋·워크트리 보존, 재오픈/재생성 시점 사용자 결정 대기. 핵클 키 교체 잔여. 피쳐 문서 `docs/features/20260611-home-skill-ranking/` |
| 데이터 | **머지 대기** | `Feat/popular-chart-ranking` (푸시됨 — 리포 Feat/ 컨벤션) | `worktrees/common-data-airflow` | ✅ 06-11 구현 `e5ec4a3` + 리뷰 수정 `ba10745`(dry-run 0.73GB 통과)·**PR [#188](https://github.com/thingsflow/common-data-airflow/pull/188)** — 사용자 검토·머지 대기. 칩 복합키 바인딩 4곳 잔여 |
...
- 🟣 **실측 후속** — ✅ 06-11 부분 완료(D-5 단위=하트·750원=5하트, eval distinct 5종, topic/intents distinct, T1, 섹션 값 바인딩). 잔여 = 교차표(T3)·시그널 SQL 대조·운영 DB 덤프(칩 복합키 4곳)·SAFE_CAST 실패율 등(dmp §8). architecture 골격은 비블로킹(R5).
