...
### 4-4. 결정 반영 (2026-06-10, 결정 라운드 2 — R-1~R-8)

§4-3의 항목들이 같은 날 사용자 결정으로 해소됨 (결정 로그 = reviews/00 §A-6):

1. **variant 신호 → 필요 (재개정)** — 실험 구조는 같은 날 피드백으로 ~~2군 통합~~ → **3군(L / C-M / C-A)** 재확정: ①기존 ②칩 UI+수동 ③칩 UI+랭킹. **키18=컨테이너 게이트 유지·흡수 + 칩 수신군 내 ②랭킹 신규 키 중첩**. 서빙 분기 = ②키 variant(**C-A만** 랭킹 fetch, C-M=AsIs 유지) — §4-2의 게이팅 코드 패턴(home.ts:198-225) 복제. 킬스위치: ②키 off=전원 C-M / 키18 off=전원 L.
2. shape 어댑터 — /architect 설계 대상 유지 (결정 불요).
3. **여유분 → over-fetch limit×2** + **배치/적재 시 미노출 스킬 제외**(서빙 visibility 필터는 최종 가드로 유지).
4. **빈 랭킹 → AsIs 폴백 + 마킹·알람** — PG 최신만+트랜잭션 교체(R-5)라 배치 실패는 전일 랭킹 유지로 자연 흡수, 진짜 빈 케이스만 폴백.
5. **모집단 → 키18 B군(칩 수신군) 내 ②키 중첩** (재개정) — 기본 배분 L50/C-M25/C-A25, 최적화는 분석 문서. 키18은 게이트로 흡수 — 재선언 불요, ② 출범 시점부터 3셀 분석(키18 단독 판독은 그때부터 혼합).

동시 확정: CL-04=percentile×전체 base 풀(1차, 최종단계 검증·보완 태스크) · CL-08=가용일수 일평균+30일 부스트(당일 피크=자연 부스트) · D-7=airflow 컴퓨트/서버 write/K8s CronJob.
