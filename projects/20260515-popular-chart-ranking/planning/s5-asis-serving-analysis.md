...
3. **B(인기점수) 주입점 = `fetchSkillListBySectionType`(home.ts L540)** — variant B일 때 **후보 fetch를 랭킹 테이블 기반으로 교체**(동일 shape 반환). ~~점수순 재정렬만~~은 lazy 경로의 limit+1 선컷 때문에 불충분 — 정밀화 2026-06-10, [구현 감사](s5-asis-implementation-audit.md) §4-1. **앱/계약 무변경, A 캡처(스냅샷) 불요.** → 초판의 CL-03 "(다) 하이브리드 스냅샷" 논의는 **무효**.
...
- **주입점 = `fetchSkillListBySectionType`(home.ts L540) — variant B일 때 후보 fetch를 랭킹 테이블 기반 조회로 교체**(동일 shape 반환) 후 기존 정형화·cap 통과. ~~직후 re-sort~~는 lazy 경로의 **limit+1 선컷**(L648: vertical=8개/horizontal=9개만 fetch) 때문에 AsIs 상위 안에서만 섞여 불충분 — 정밀화 2026-06-10, [s5-asis-implementation-audit.md](s5-asis-implementation-audit.md) §4-1.
...
- **A 캡처(스냅샷) 불필요** — A는 오늘도 매 요청 live 계산. B는 batch 선계산(랭킹 테이블). **A/B = "live AsIs" vs "batch 인기점수"**.
  ...
  - 정정: **A = 현행 live(featuredSkillsTabs/fetchSkillListBySectionType) 유지, B = 후보 fetch 교체(점수순·동일 shape).** 스냅샷 논의 불필요.
...
| **B 주입 seam(후보 fetch 교체)** | `src/services/home.ts` `fetchSkillListBySectionType` L540 (lazy 호출부 L641, limit+1 L648 / tagSkills+subKey L558~573, recentPurchased L590) |
