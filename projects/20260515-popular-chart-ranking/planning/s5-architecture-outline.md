...
> 스텝: S5 (아키텍처 설계) · 작성: 2026-06-08 · 상태: **✅ 완료** — 윤곽(06-08) → **세부 패스 완료(06-10: [architecture.md](../architecture.md) · [api-spec.md](../api-spec.md))**
...
> 구현 감사(2026-06-10): [s5-asis-implementation-audit.md](s5-asis-implementation-audit.md) — 피쳐 문서↔코드 전수 대조 + A 정밀 정의 + ⭐B seam 정밀화(후보 fetch 교체)
>
...
| ② | **랭킹 산출물 + 리버스-ETL** | 마트 row 형상 · BQ→PG 적재 토폴로지(D-7) · 복합키 | row=(menu_seq, section_key, variant, score, rank, computed_date). 컴퓨트=airflow / write=서버 소유 + 스케줄러(K8s CronJob) | 토폴로지 제안됨. **A 캡처 불요(현행 live), B=후보 fetch 교체(§2)** |
...
**B(인기점수) 주입 = 단일 seam에서 후보 fetch 교체** — variant B일 때 `fetchSkillListBySectionType`이 칩의 복합키로 **랭킹 테이블 기반 후보를 점수순으로 가져와 기존 섹션과 동일 shape로 반환**한다. (정밀화 2026-06-10: lazy 경로가 limit+1로 선컷하므로 "fetch 후 재정렬"은 AsIs 상위 8~9개 안의 순열일 뿐 — [구현 감사](s5-asis-implementation-audit.md) §4-1. 확정사항 "B는 별도 풀·후보풀 비공유"와도 이쪽이 정합)

| 항목 | 결론 |
|---|---|
| A 캡처/스냅샷 | **불요** — A는 현행 live 경로 유지 |
| B 주입점 | `fetchSkillListBySectionType`(home.ts L540) **후보 fetch 교체**(동일 shape 반환, 단일 seam) — limit+1 선컷으로 post-fetch 재정렬 불가 |
| 앱/계약 변경 | **불요** — 앱은 순서대로 받아 위치 뱃지만 부여 |
| A/B 성격 | "live AsIs 순서" vs "batch 인기점수 순서" |
| variant 신호 | **재개정(06-10, 피드백): 3군 L/C-M/C-A** — 키18은 컨테이너 게이트로 유지·흡수, 칩 수신군 내부에 **②랭킹 신규 키 중첩**(C-M=수동 유지 ↔ C-A=랭킹). 서빙 분기=②키 variant(**C-A만** 랭킹 fetch) + 빈 랭킹 AsIs 폴백 + 킬스위치(②키 off=전원 C-M / 키18 off=전원 L) |
...
```
B fetch-교체 seam(§2) ─┬─► ②테이블 형상 ──► ③랭커/주입점 ──► ④서빙 API
                       └─► ①계산(norm·cold-start) ─────────────┘
                                          ⑤측정(event-spec) — ④와 병행, B 출시와 동시
배치 토폴로지(D-7) ──► ② 적재 메커니즘 확정
```

- **먼저**: ② 산출물 형상(랭킹 테이블) + 토폴로지 확정 (구조 척추) — B는 `fetchSkillListBySectionType` 후보 fetch 교체(§2)
...
| ID | 결정 | 권장 | 비고 |
|----|------|------|------|
| **CL-03** | A 비교군 산출 방식 | **캡처 불요 — A=현행 live**, B=후보 fetch 교체(§2). ②랭킹 variant 주입점만 ③ | 정정 06-08(LIVE) · 정밀화 06-10(limit+1 선컷) |
| **CL-04** | `norm` 방식·모집단 | ✅ **확정(06-10)**: percentile × 전체 base 풀 (1차) | 최종단계 데이터 검증·보완 별도 태스크 |
| **CL-08** | 신규 섹션 cold-start | ✅ **확정(06-10)**: 가용일수 일평균(÷min(7,경과일)) + 30일 부스트 | 당일 피크=자연 부스트 수용 |
| **DF-S4-1** | 태그 소스(임시 승격 vs 공식 흡수) | 1차=임시 태그 그대로, 정식 승격 후속(하이브리드) | 필터 레이어 운영 형식과 연동 |
| **D-7** | 배치 토폴로지 | ✅ **확정(06-10)**: 컴퓨트=airflow / write=서버 / K8s CronJob | — |
...
B 후보 fetch 교체 seam 확정(§2, [구현 감사](s5-asis-implementation-audit.md)) → `/architect` 세부 패스로 진입, **②→①③④→⑤** 순서로 [architecture.md](../architecture.md)·[api-spec.md](../api-spec.md) 작성. ②랭킹 variant 주입점(③)·랭킹 테이블·base/open_date가 핵심. 세부 작성 시 본 윤곽의 영역 구분과 결정점(§4)을 그대로 이어받는다.

> ✅ **2026-06-10 세부 패스 완료** — architecture.md(5레이어 세부 + 신규 설계 결정 8건 §8) · api-spec.md(적재 PUT·lazy 내부 분기·DDL·플래그/키) v1 작성. 본 윤곽 문서는 derivation 기록으로 보존 — 이후 정본은 architecture.md.
