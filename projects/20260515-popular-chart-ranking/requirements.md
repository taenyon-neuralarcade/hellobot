...
> 작성: 2026-06-05 (코디네이터) · 갱신: 2026-06-06 · 상태: **v3 (기획 피드백 + 현황조사 반영)**
...
> ⚠️ **CL-23(가설↔확정 분리)**: 2026-06-06 결정으로 신규 기준(open_date≤6개월)·실시간 윈도우(공통7일)·1:1상담·targets(최종단계)·base(오리지널∧750원)는 **확정**. 잔여 실측 종속 = **base 가격 단위 바인딩(원 vs 하트)·`eval_emoji` distinct·섹션 태그 값 바인딩**뿐 — 이 잔여만 실측 후 확정, 나머지 FR은 확정 계약.
...
| ID | 요구사항 | 등급 | 근거 |
|----|----------|------|------|
| **FR-F0** | **기본 풀 조건(base)** = **오리지널(`original_type='original'`) ∧ 유료 ≥750원** (✅ 확정 2026-06-06). 전 섹션 공통 선결 필터. ⚠️ 현행 '지금 인기'는 minPrice 60(하트)이라 **별도 임계** — 750원을 BQ 마트 가격 필드·단위(원 vs 하트)에 바인딩(/dev-data·D-5). | MUST | 기획 확정 |
...
| **FR-F6** | 필터 규칙(섹션→조건식)은 **코드 변경 없이 조건 추가·수정 가능한 확장 구조**를 우선한다. 1차 저장 위치(코드 상수/설정/DB seed, API단 하드코딩 가능성 포함)는 S5 확정, 어드민 편집 UI는 후속(CL-16). | SHOULD | DF-S4-3·기획 피드백 |
| **FR-F7** | 각 섹션 필터의 실제 태그 **값 바인딩**(예: '애정운'/'재회')은 /dev-data 실측(distinct 값)으로 확정한 뒤 적용한다. | MUST | s4 §6 |
...
| **FR-R3** | B 랭커 점수는 다음 가중합이어야 한다: 구매수0.35 + 조회수0.1 + 전환율0.2 + 긍정평가비율0.15 + 매출0.15 + 신규부스트0.05 (**6항목 확정** — α 결정 2026-06-05). 매출 = `revenue_krw` (외부채널 포함 수용 — CL-07). ⚠️ **정규화 `norm()` 방식·모집단·결측 = S5 위임**(CL-04, 미정 시 순위 비결정; 권장=percentile·전체풀). | MUST(norm은 S5) | s3 결론 |
| **FR-R4** | 점수는 **7일 윈도우**(최근 7일 합 ÷ 실데이터 일수)로 산출하며 **일배치**로 갱신한다. **'실시간 인기' 섹션도 공통 7일 점수를 사용**(별도 단기 윈도우 없음, 명칭은 라벨 — CL-09). | MUST | s3 |
| **FR-R8** | **신규 인기 섹션 = 출시일 `open_date` ≤ 6개월**(N 기본 6개월, 결과 보고 가변·C-2). 출시일 소스 = `mart_skill_open_date_se.event_date`(D-6). 신규 섹션 랭킹·cold-start는 S5 위임(CL-08). | MUST(N 가변) | C-2/D-6 |
| **FR-R9** | A·B는 **동일 후보 풀이 아니다**(A=현행 산출 전체, B=신규 필터+랭커·C-1). A/B 비교는 *같은 풀의 정렬 차이*가 아니라 **섹션 단위 행동 KPI**(노출→클릭→구매→총 매출)로 한다. A(AsIs) 산출·캡처 방식은 S5 위임(CL-03, 권장=배치 스냅샷). | MUST(방식 S5) | C-1/CL-03 |
| **FR-R10** | B 랭커도 **pinned 상위 고정을 유지**한 뒤 나머지를 점수 정렬한다(현행 운영 기능 보존·회귀 방지, CL-14). | MUST | CL-14 |
| **FR-R5** | 긍정평가비율 = **`1 − 💩비율`** = `AVG(CASE WHEN eval_emoji='💩' THEN 0 ELSE 1 END)` (0~1, 별점 부재로 재정의). ⚠️ line216(1~5 평균) 폐기, `eval_emoji` distinct 실측 필요(CL-06). **신규부스트 = 출시일 `open_date` 30일 이내 가산**(D-4, 소스 `mart_skill_open_date_se.event_date`·서버 등록일 폐기). | MUST(식 확인) | s3·D-4/D-6 |
...
| **FR-AB4** | A/B 버킷팅·실험 배포의 **구현은 이번 범위 밖**이다(컨테이너 핵클 키18 A/B 종료 후 별도·순차). 이번엔 주입점만 비워 둔다. | MUST | DF-S2-2 |
...
| **FR-B3** | 리버스-ETL 적재 주체 **제안(TODO-042 조사 후)**: **서버 소유 write**(rankings 테이블·트랜잭션을 서버가 소유) + **스케줄러 트리거**(K8s CronJob 권장=마이그레이션 정합 / 단기 mwaa cron 가능) — 기존 `update_collection_ranking`·`refresh_recent_purchased_skills` 컨벤션과 일관. common-data-airflow 직접 PG write는 비권장(역할 경계·운영DB 위험·psycopg2 부재). 확정=S5. | SHOULD | s3·D-7 |
| **FR-B4** | 배치는 **D+1 일배치**로 동작한다(기존 마트 체인 KST 11시 산출과 정합; 기획 '새벽4시'와의 타이밍은 S5에서 조정). | MUST(타이밍 확인) | s3 DF-S3-5 |
...
| **NFR-2 신선도/폴백** | 노출 목록은 일배치(D+1) 기준 최신. **(a) 배치 실패** → 직전 결과 폴백. **(b) 정상 배치·후보 슬롯 미달/0(CL-10)** → **해당 섹션 숨김**(저품질 채움·빈 노출 금지). | MUST |
| **NFR-3 정합** | 적재 결과의 menu_seq는 서비스 DB의 유효·노출가능 스킬과 정합해야 한다(삭제·비노출 스킬 제외). | MUST |
...
- **C2** A/B 실험 메커니즘(버킷팅)은 이번 구현 범위 밖.
- **C3** 평점은 별점 데이터 부재 — 긍정평가비율로 대용.
- **C4** BQ→서비스 PG reverse-ETL 기존 경로 없음 — 신규 구축 필요.
- **C5** 가중치(α)·조회 분모·Phase A(β)는 기획 확인 전까지 가설값.
