...
- [`TODO.md`](TODO.md) — **표 형식 인덱스**. 5개 섹션(모니터링 / 진행 중 / 대기 / 프로젝트로 추적 / 완료)으로 분리. 한 행이 한 항목, ID·제목·다음 액션·마감·상세 컬럼.
...
3. `TODO.md`의 해당 섹션 표에 한 행 추가:
   - 모니터링: `| TODO-NNN | 제목 | 다음 체크 시점 | 등록일 | 시작일 | 마감 | [상세] |`
   - 액션·대기: `| TODO-NNN | 제목 | 다음 액션 | 등록일 | 시작일 | 마감 | [상세] |`
4. **동시에** `todos/TODO-NNN-{slug}.md` 생성 ([todos/readme.md](todos/readme.md) 템플릿 사용). 사소한 일도 예외 없이 생성.
5. 상세 파일의 `컨텍스트`에 사용자 요청 원문·배경·관련 링크 기록
...
- **완료**: TODO.md 행을 `완료 (최근 10건)` 섹션으로 이동. 완료 섹션 컬럼은 `ID | 제목 | 등록일 | 시작일 | 완료일 | 상세` (다음 액션·마감 컬럼 없음). 상세 파일 frontmatter `상태` + `완료` 필드 갱신 후 보존.
